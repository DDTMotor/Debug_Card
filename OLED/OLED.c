...
// 随后调用OLED_Update函数或OLED_UpdateArea函数,才会将显存数组的数据发送到OLED硬件，进行显示
uint8_t g_oled_dispalybuff[8][128];
 ...
 * ******************************************************************************
 * @brief 	: OLED写SCL高低电平
 * @param 	: BitValue [in/out],要写入SCL的电平值，范围：0/1
 * @author 	: chenningzhan
 * @note	: None
 * ******************************************************************************
 */
 ...
 * ******************************************************************************
 * @brief 	: OLED写SDA高低电平
 * @param 	: BitValue [in/out], 要写入SDA的电平值，范围：0/1
 * @author 	: chenningzhan
 * @note	: None
 * ******************************************************************************
 */
 ...
 * ******************************************************************************
 * @brief 	: I2C发送一个字节
 * @param 	: Byte [in/out], 要发送的一个字节数据，范围：0x00~0xFF
 * @author 	: chenningzhan
 * @note	: None
 * ******************************************************************************
 */
 ...
 * ******************************************************************************
 * @brief 	: OLED写命令
 * @param 	: Command [in/out], 要写入的命令值，范围：0x00~0xFF
 * @author 	: chenningzhan
 * @note	: None
 * ******************************************************************************
 */
...
{
    OLED_I2C_Start();
    OLED_I2C_SendByte(OLED_IIC_ADDR);
...
void OLED_Update(void)
{
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        OLED_WriteData(g_oled_dispalybuff[j], 128);
    }
}
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Page; j < Page1; j++)
    {
        if (X >= 0 && X <= 127 && j >= 0 && j <= 7) // 超出屏幕的内容不显示
        {
            ...
            OLED_WriteData(&g_oled_dispalybuff[j][X], Width);
        }
    }
}
...
void OLED_Clear(void)
{
    ...
    for (j = 0; j < 8; j++) // 遍历8页
    {
        ...
        {
            g_oled_dispalybuff[j][i] = 0x00; // 将显存数组数据全部清零
        }
    }
}
...
void OLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Y; j < Y + Height; j++) // 遍历指定页
    {
        for (i = X; i < X + Width; i++) // 遍历指定列
        {
            ...
            {
                g_oled_dispalybuff[j / 8][i] &= ~(0x01 << (j % 8)); // 将显存数组指定数据清零
            }
        }
    }
}
...
void OLED_Reverse(void)
{
    ...
    for (j = 0; j < 8; j++) // 遍历8页
    {
        for (i = 0; i < 128; i++) // 遍历128列
        {
            g_oled_dispalybuff[j][i] ^= 0xFF; // 将显存数组数据全部取反
        }
    }
}
...
void OLED_ReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Y; j < Y + Height; j++) // 遍历指定页
    {
        for (i = X; i < X + Width; i++) // 遍历指定列
        {
            if (i >= 0 && i <= 127 && j >= 0 && j <= 63) // 超出屏幕的内容不显示
            {
                g_oled_dispalybuff[j / 8][i] ^= 0x01 << (j % 8); // 将显存数组指定数据取反
            }
        }
    }
}
    ...
    for (j = 0; j < (Height - 1) / 8 + 1; j++)
    {
        ...
        for (i = 0; i < Width; i++)
        {
            if (X + i >= 0 && X + i <= 127) // 超出屏幕的内容不显示
            {
                ...
                {
                    /*显示图像在当前页的内容*/
                    g_oled_dispalybuff[Page + j][X + i] |= Image[j * Width + i] << (Shift);
                }
                if (Page + j + 1 >= 0 && Page + j + 1 <= 7) // 超出屏幕的内容不显示
                {
                    /*显示图像在下一页的内容*/
                    g_oled_dispalybuff[Page + j + 1][X + i] |= Image[j * Width + i] >> (8 - Shift);
                }
            }
        }
    }
...
void OLED_DrawPoint(int16_t X, int16_t Y)
{
    ...
    {
        /*将显存数组指定位置的一个Bit数据置1*/
        g_oled_dispalybuff[Y / 8][X] |= 0x01 << (Y % 8);
    }
}
...
uint8_t OLED_GetPoint(int16_t X, int16_t Y)
{
    ...
    {
        /*判断指定位置的数据*/
        if (g_oled_dispalybuff[Y / 8][X] & 0x01 << (Y % 8))
        {
            return 1; // 为1，返回1
        }
    }
